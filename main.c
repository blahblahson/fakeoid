...
#define ADDR(mem, a) ((uint8_t *)&mem[(a)])
#define PAGE(mem, n) (ADDR((mem), (n)*0xFF))
#define ZEROPAGE (PAGE(mem, 0))

#define ADDR_INDIRECT(mem, a) ((uint16_t)(mem[a] + (mem[a + 1] << 8)))
#define BIT(n) (1u << (n))
...
struct registers
{
    ...
    struct procstat
    {
        ...
        bool v : 1; /* overflow */
        bool n : 1; /* negative */
    } p; /* TODO: processor status - represent as bitfield? */
} reg = { 0 };

/*
 * Order of bitfields is implementation defined, so we need a helper function
 */
uint8_t procstat_to_word(struct procstat p)
{
    uint8_t word = 0;
    word |= p.c << 0;
    word |= p.z << 1;
...
struct procstat word_to_procstat(uint8_t word)
{
    return (struct procstat){
        .c = word & BIT(0),
        .z = word & BIT(1),
        .i = word & BIT(2),
        .d = word & BIT(3),
...
uint8_t load(const operand_t operand)
{
    switch (operand.type)
    {
...
uint16_t addr(const operand_t operand)
{
    switch (operand.type)
    {
    case OPERAND_TYPE_ADDRESS:
...
void store(const operand_t operand, const word_t word)
{
    switch (operand.type)
    {
    case OPERAND_TYPE_ADDRESS:
        mem[operand.addr] = word;
        break;
    case OPERAND_TYPE_ACCUMULATOR:
        reg.a = word;
        break;
...
static void push16(uint16_t dword)
{
    /* TODO: overflow? */
    stack[reg.s--] = dword & 0xFF;
    stack[reg.s--] = (dword >> 8) & 0xFF;
}
...
{
    /* FIXME: (cppcheck) Expression '(stack[++reg.s]<<8)|stack[++reg.s]' depends on order of
     * evaluation of side effects */
    return (stack[++reg.s] << 8) | stack[++reg.s];
}
...
void bbr_(operand_t operand, int nr)
{
    /* TODO: review this */
...
void bbr0(operand_t operand)
{
    bbr_(operand, 0);
}

void bbr1(operand_t operand)
{
    bbr_(operand, 1);
}

void bbr2(operand_t operand)
{
    bbr_(operand, 2);
}

void bbr3(operand_t operand)
{
    bbr_(operand, 3);
}

void bbr4(operand_t operand)
{
    bbr_(operand, 4);
}

void bbr5(operand_t operand)
{
    bbr_(operand, 5);
}

void bbr6(operand_t operand)
{
    bbr_(operand, 6);
}

void bbr7(operand_t operand)
{
    bbr_(operand, 7);
}

/* BBS: Branch on bit set */
void bbs_(operand_t operand, int nr)
{
    /* TODO: review this */
    if (load(operand) & BIT(nr))
        reg.pc = addr(operand);
...
void bbs0(operand_t operand)
{
    bbs_(operand, 0);
}
void bbs1(operand_t operand)
{
    bbs_(operand, 1);
}
void bbs2(operand_t operand)
{
    bbs_(operand, 2);
}
void bbs3(operand_t operand)
{
    bbs_(operand, 3);
}
void bbs4(operand_t operand)
{
    bbs_(operand, 4);
}
void bbs5(operand_t operand)
{
    bbs_(operand, 5);
}
void bbs6(operand_t operand)
{
    bbs_(operand, 6);
}
void bbs7(operand_t operand)
{
    bbs_(operand, 7);
}

/* BCC: Branch if carry clear */
void bcc(operand_t operand)
{
    if (!reg.p.c)
        reg.pc = addr(operand);
}
...
{
    uint8_t result = reg.a - load(operand);
...
{
    uint8_t result = reg.x - load(operand);
...
{
    uint8_t result = reg.y - load(operand);
...
void rmb_(operand_t operand, int nr)
{
    store(operand, load(operand) & ~BIT(nr));
}

void rmb0(operand_t operand)
{
    rmb_(operand, 0);
}

void rmb1(operand_t operand)
{
    rmb_(operand, 1);
}

void rmb2(operand_t operand)
{
    rmb_(operand, 2);
}

void rmb3(operand_t operand)
{
    rmb_(operand, 3);
}

void rmb4(operand_t operand)
{
    rmb_(operand, 4);
}

void rmb5(operand_t operand)
{
    rmb_(operand, 5);
}

void rmb6(operand_t operand)
{
    rmb_(operand, 6);
}

void rmb7(operand_t operand)
{
    rmb_(operand, 7);
}
...
/* SMB: Set memory bit */
void smb_(operand_t operand, int nr)
{
    store(operand, load(operand) | BIT(nr));
}

void smb0(operand_t operand)
{
    smb_(operand, 0);
}

void smb1(operand_t operand)
{
    smb_(operand, 1);
}

void smb2(operand_t operand)
{
    smb_(operand, 2);
}

void smb3(operand_t operand)
{
    smb_(operand, 3);
}

void smb4(operand_t operand)
{
    smb_(operand, 4);
}

void smb5(operand_t operand)
{
    smb_(operand, 5);
}

void smb6(operand_t operand)
{
    smb_(operand, 6);
}

void smb7(operand_t operand)
{
    smb_(operand, 7);
}
...
/* STZ: Store zero */
void stz(operand_t operand)
{
    store(operand, 0);
}

/* TAX: Transfer accumulator to X */
void tax(operand_t operand)
{
    (void)operand;
    reg.x = reg.a;
...
op_desc_t ops[] = {
    ...
    /* TODO: Add now BBRn instructions, which actually use a combination of the addressing modes
       zero page and relative: zp,r. TODO: Check if the 3rd byte (relative) is even read if branch
       condition is false. See http://6502.org/tutorials/65c02opcodes.html for more info. */
...
{
    memcpy(ADDR(mem, VECTOR_RESET), prog, sizeof(prog));
}
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        case ADDR_MODE_ABSOLUTE_INDEXED_INDIRECT:
            operand.addr = mem[shift() + (shift() << 8) + reg.x];
            break;
        ...
        case ADDR_MODE_ABSOLUTE_INDIRECT:
            operand.addr = mem[shift() + (shift() << 8)];
            break;
        ...
        case ADDR_MODE_ZEROPAGE_INDEXED_INDIRECT:
            operand.addr = mem[(uint8_t)(shift() + reg.x)];
        ...
        case ADDR_MODE_ZEROPAGE_INDIRECT:
        {
            ...
            operand.addr = mem[zp_offset] + (mem[zp_offset + 1] << 8);
            break;
        }
        case ADDR_MODE_ZEROPAGE_INDIRECT_INDEXED:
        {
            /* TODO: zp_offset+1 should wrap around? */
            uint8_t zp_offset = shift();
            operand.addr = mem[zp_offset] + (mem[zp_offset + 1] << 8) + reg.y;
            break;
        }
